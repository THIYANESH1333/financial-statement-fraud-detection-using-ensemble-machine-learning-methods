...
def create_perfectly_aligned_tfidf_flowchart():
    """Create a perfectly aligned TF-IDF flowchart with precise positioning"""
    ...
    try:
        ...
        font_pipeline = ImageFont.truetype("arial.ttf", 22)
    except:
        try:
            font_title = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", 40)
            font_label = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", 24)
            font_desc = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", 18)
            font_pipeline = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", 22)
        except:
    ...
    def draw_arrow(x1, y1, x2, y2, fill=0, width=5):
        # Draw main line
        ...
        # Draw arrowhead
        draw.polygon([(x2, y2), (x3, y3), (x2, y2)], fill=fill, outline=fill)
    
    # Function to draw L-shaped arrow
    def draw_l_arrow(x1, y1, x2, y2, x3, y3, fill=0, width=5):
        # Draw first segment
        draw.line([(x1, y1), (x2, y2)], fill=fill, width=width)
        # Draw second segment
    ...
    draw_centered_text(width//2, 60, title_text, font_title)
    ...
    for center_x, center_y, label, desc in boxes:
        ...
        # Draw label
        draw_centered_text(center_x, center_y - 25, label, font_label)
        
        # Draw description
        draw_centered_multiline(center_x, center_y + 25, desc, font_desc)
    ...
    draw_centered_text(width//2, height - 50, pipeline_text, font_pipeline)
    
    # Save the image
    img.save('TF-IDF_Feature_Extraction_Process_Fixed.png')
    print("Perfectly aligned TF-IDF flowchart created successfully!")
    print("File saved: TF-IDF_Feature_Extraction_Process_Fixed.png")

if __name__ == "__main__":
    create_perfectly_aligned_tfidf_flowchart()
