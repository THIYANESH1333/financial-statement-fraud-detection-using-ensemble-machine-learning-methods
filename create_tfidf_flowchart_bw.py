...
def create_black_white_tfidf_flowchart():
    """Create a pure black and white TF-IDF flowchart"""
    
    # Create a larger image for better clarity
    width, height = 1400, 900
    img = Image.new('L', (width, height), 255)  # Grayscale mode, white background
    ...
    draw.text((width//2 - title_width//2, 20), title_text, fill=0, font=font_title)  # Black text
    ...
    for x, y, label, desc in all_boxes:
        ...
        draw.rectangle([x, y, x+box_width, y+box_height], outline=0, width=3)  # Black outline
        
        # Draw label (centered)
        label_bbox = draw.textbbox((0, 0), label, font=font_label)
        label_width = label_bbox[2] - label_bbox[0]
        draw.text((x + box_width//2 - label_width//2, y + 10), label, fill=0, font=font_label)  # Black text
        
        # Draw description (centered)
        desc_bbox = draw.textbbox((0, 0), desc, font=font_desc)
        desc_width = desc_bbox[2] - desc_bbox[0]
        draw.text((x + box_width//2 - desc_width//2, y + 45), desc, fill=0, font=font_desc)  # Black text
    
    # Draw improved arrows with arrowheads
    def draw_arrow(x1, y1, x2, y2, arrow_size=8):
        # Draw main arrow line
        draw.line([x1, y1, x2, y2], fill=0, width=4)  # Black line
        ...
        if length > 0:
            ...
            # Draw arrowhead
            draw.polygon([(x2, y2), (arrow_x1, arrow_y1), (arrow_x2, arrow_y2)], fill=0)  # Black arrowhead
    ...
    # Draw background for pipeline text (light gray in grayscale)
    draw.rectangle([width//2 - pipeline_width//2 - 20, height - 60, 
                   width//2 + pipeline_width//2 + 20, height - 20], 
                  fill=200, outline=0, width=2)  # Light gray background, black border
    
    draw.text((width//2 - pipeline_width//2, height - 50), pipeline_text, 
              fill=0, font=font_pipeline)  # Black text
    
    # Add a subtle border around the entire flowchart
    draw.rectangle([20, 80, width-20, height-80], outline=128, width=2)  # Gray border
    
    # Save the black and white image
    img.save('TF-IDF_Feature_Extraction_Process_BW.png')
    print("Black and White TF-IDF Feature Extraction Process flowchart created successfully!")
    print("File saved: TF-IDF_Feature_Extraction_Process_BW.png")
    
    # Also create a PDF version
    try:
        img.save('TF-IDF_Feature_Extraction_Process_BW.pdf')
        print("PDF version also saved: TF-IDF_Feature_Extraction_Process_BW.pdf")
    except:
        print("PDF version could not be created (PIL PDF support may not be available)")

if __name__ == "__main__":
    create_black_white_tfidf_flowchart()
